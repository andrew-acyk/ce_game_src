/*
End of the game process (developer: Andrew) :
Tables/data to be set at the end of the game:
ce_carbonexch_final_summary, ce_plant_model_data_points, ce_plant_model_statistics, 

- create one ce_carbonexch_final_summary record for each account id and add/calculate values of the following fields
  game_id, game_start_date, account_id, plant_id, total_costs_per_unit_period_tcpup, adjusted_costs_per_unit_acpu (continue the rest of the calculations later on)
- create one ce_plant_model_data_points record for each account_id in the game and add values to the following fields
  game_id, game_start_date, account_id, plant_id, adjusted_costs_per_unit_acpu
- create on ce_plant_model_statistics record for each plant and add/calculate values of the following fields
  plant_id, number_of_samples, average_of_acpu, std_dev
  Number_of_samples = number of records in ce_plant_model_data_points for that plant_id
  average_of_acpu = sum(ce_plant_model_data_points.adjusted_codst_per_unit_acpu)/number_of_samples
- Re-calculate weight_factor_for_ranking in ce_plant table
  weight_factor_for_ranking =ave(acpup of top tier samples of all plants)/ave(acpup of top tier samples for this plant) * ce_baseln_acpup_eq_what_perc
- back to the ce_carbonexch_final_summary record for each account id and calculate the remaining fields
  adjusted_costs_per_unit_percentile_acpup, normalized_costs_per_unit_percentile (only calculated for the top rank)
  adjusted_costs_per_unit_percentile_acpup = find_percentile(ce_plant_model_statistics.average_of_acpu, ce_plant_model_statistics.std_dev, adjusted_costs_per_unit_acpu)
  normalized_costs_per_unit_percentile = acpup of top rank of this plant * weight_factor_for_ranking
