...
add_to_ce_game_query = ("INSERT INTO ce_game (game_id, game_name, game_desc, game_type, game_start_date, market_id, created_date, created_by, updated_date, updated_by) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)")
...
loginids = []
membernames = {} #dictionary with keys: account id and values: names of members
...
while line:
    if line[0] == "#":
        ...
        if attr[0].strip() == "GAME ID": #check whether game ID is a positive value
            ...
            gameid = attr[1].strip()
        else if "GAME NAME" in attr[0].strip():
            gamename = attr[1].strip()
        else if "GAME DESCRIPTION" in attr[0].strip():
            ...
            membernames[attr[1].split(":")[0]] = attr[1].split(":")[1].strip()
        ...
        else if attr[0].strip() == "COMPANY ID (ONE COMPANY PER ACCOUNT)": #check whether same number as account IDs
            if repeats(attr[1].strip().split(",")):
                raise Exception("Repeated Company ID.")
            else if len(attr[1].strip().split(",")) != numAccounts:
                raise Exception("Different number of Company IDs than Account IDs.")
            companyids = attr[1].strip().split(",")
        else if attr[0].strip() == "PLANT ID (ID EXISTS IN CE_PLANY TABLE)": #check whether ID exists in the plant table already and check whether there are at least as many plant IDs as account IDs
            if repeats(attr[1].strip().split(",")):
...
i = 0
while i < numAccounts:
    ce_account_columns = (accountids[i], "team" + accountids[i], "team" + accountids[i], loginids[i], membernames[i], created_date, created_by, updated_date, updated_by)
