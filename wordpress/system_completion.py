...
def run(game_id, game_start_date):
    ...
    carbon_price = ''
    sys_completn_charge_perc = ''
    updated_by = ''
    carbon_price_last_perd = 0

    cursor1.execute(carbon_price_query, (game_id))
    for row in cursor1:
        carbon_price = row['carbon_price_last_perd']

    cursor1.execute(sys_completn_charge_perc_query)
    for row in cursor1:
        sys_completn_charge_perc = row['parameter_value']

    cursor1.execute(updated_by_system_query)
    for row in cursor1:
        updated_by = row['parameter_value']

    cursor1.execute(carbon_price_last_perd_query, (game_id))
    for row in cursor1:
        carbon_price_last_perd = float(row['carbon_price_last_perd'])
